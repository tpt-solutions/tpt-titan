...
export class EditorHistory {
	 ...
	 * @param {string} state.type - Editor type ('blocks', 'markdown', 'richtext')
	 * @param {*} state.content - The content (blocks array, markdown string, or HTML)
	 * @param {Object} state.metadata - Additional metadata (cursor position, selection, etc.)
	 */
	push(state) {
		// Don't save if we're currently undoing/redoing
	...
	undo() {
		...
		// Use setTimeout to reset the flag after the current operation
		setTimeout(() => {
			this.isUndoing = false;
		}, 0);
 ...
 * @param {EditorHistory} history - The history instance
 * @param {number} delay - Debounce delay in ms
 * @returns {Function}
 */
export function createDebouncedPush(history, delay = 500) {
	let timeoutId = null;
	let lastState = null;
	...
	return (state) => {
		...
		const stateString = JSON.stringify(state.content);
