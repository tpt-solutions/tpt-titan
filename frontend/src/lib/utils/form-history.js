...
export class FormHistory {
	 ...
	 * @param {Array} state.fields - Array of form fields
	 * @param {Object} state.settings - Form settings
	 * @param {string} state.actionType - Type of action ('addField', 'deleteField', 'updateField', 'reorderFields', etc.)
	 * @param {string} state.description - Human-readable description of the action
	 */
	push(state) {
		...
		// Add new state with deep clone to prevent reference issues
		this.history.push({
			...state,
 ...
 * @param {FormHistory} history - The history instance
...
export function createDebouncedPush(history, delay = 500) {
	...
	return (state) => {
		...
		const stateString = JSON.stringify(state.fields);
