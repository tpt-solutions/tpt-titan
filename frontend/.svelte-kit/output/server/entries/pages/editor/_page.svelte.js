...
const TextEditorToolbar = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `  <div class="bg-white border-b border-gray-200"> <div class="px-6 py-3 border-b border-gray-100"><div class="max-w-6xl mx-auto flex items-center justify-between"><div class="flex items-center space-x-3 flex-1 min-w-0"><input placeholder="Document title..." class="text-xl font-semibold bg-transparent border-none outline-none focus:ring-2 focus:ring-blue-500 rounded px-2 py-1 w-full max-w-md"${add_attribute("value", documentTitle2, 0)}> <div class="flex items-center space-x-2 shrink-0">${hasUnsavedChanges ? `<span class="text-xs text-orange-600 font-medium" data-svelte-h="svelte-o526t3">● Unsaved</span>` : ``} ${saveStatus ? `<span class="text-xs text-gray-400">${escape(saveStatus)}</span>` : ``}</div></div></div></div>  <div class="px-6 py-2 bg-gray-50"><div class="max-w-6xl mx-auto flex items-center justify-between flex-wrap gap-2"> <div class="flex items-center space-x-1"><button class="px-3 py-1.5 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 hover:border-gray-400 transition-colors flex items-center space-x-1" title="Create new document" data-svelte-h="svelte-1we529j"><span>➕</span> <span>New</span></button> <button class="px-3 py-1.5 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 hover:border-gray-400 transition-colors flex items-center space-x-1" title="Open existing document" data-svelte-h="svelte-1myncxw"><span>📂</span> <span>Open</span></button> <button class="px-3 py-1.5 text-sm font-medium text-white bg-blue-600 border border-blue-600 rounded-md hover:bg-blue-700 transition-colors flex items-center space-x-1 disabled:opacity-50 disabled:cursor-not-allowed" ${isSaving ? "disabled" : ""} title="Save document (Ctrl+S)"><span data-svelte-h="svelte-kjmw7x">💾</span> <span>${escape(isSaving ? "Saving..." : "Save")}</span></button>  <div class="relative"><button class="px-3 py-1.5 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 hover:border-gray-400 transition-colors flex items-center space-x-1" title="Export document" data-svelte-h="svelte-xy7lcs"><span>📤</span> <span>Export</span> <span class="text-xs ml-1">▼</span></button> ${``}</div></div>  <div class="flex items-center bg-white rounded-lg border border-gray-300 p-1 shadow-sm"><button class="${"px-3 py-1 text-sm font-medium rounded-md transition-all duration-200 " + escape(
    editorMode === "blocks" ? "bg-blue-600 text-white shadow-sm" : "text-gray-600 hover:bg-gray-100",
    true
  )}" title="Block-based editor (Notion-style)">🧱 Blocks</button> <button class="${"px-3 py-1 text-sm font-medium rounded-md transition-all duration-200 " + escape(
    editorMode === "markdown" ? "bg-blue-600 text-white shadow-sm" : "text-gray-600 hover:bg-gray-100",
    true
  )}" title="Markdown editor">📝 Markdown</button> <button class="${"px-3 py-1 text-sm font-medium rounded-md transition-all duration-200 " + escape(
    editorMode === "richtext" ? "bg-blue-600 text-white shadow-sm" : "text-gray-600 hover:bg-gray-100",
    true
  )}" title="Rich text editor (Word-style)">✨ Rich Text</button></div>  <div class="flex items-center space-x-1"> <div class="flex items-center space-x-1 pr-2 border-r border-gray-300"><button class="p-1.5 text-gray-600 hover:bg-gray-200 rounded-md transition-colors disabled:opacity-30 disabled:cursor-not-allowed" ${!canUndo ? "disabled" : ""} title="Undo (Ctrl+Z)">↩️</button> <button class="p-1.5 text-gray-600 hover:bg-gray-200 rounded-md transition-colors disabled:opacity-30 disabled:cursor-not-allowed" ${!canRedo ? "disabled" : ""} title="Redo (Ctrl+Y)">↪️</button> <button class="p-1.5 text-gray-600 hover:bg-gray-200 rounded-md transition-colors" title="Find &amp; Replace (Ctrl+F)" data-svelte-h="svelte-1xgy7pu">🔍</button></div>  <div class="relative"><button class="px-3 py-1.5 text-sm font-medium text-indigo-700 bg-indigo-50 border border-indigo-200 rounded-md hover:bg-indigo-100 transition-colors flex items-center space-x-1" title="AI Writing Assistant" data-svelte-h="svelte-1j98w9s"><span>🤖</span> <span>AI</span> <span class="text-xs ml-1">▼</span></button> ${``}</div>  <div class="flex items-center space-x-1 pl-2 border-l border-gray-300"><button class="p-1.5 text-gray-600 hover:bg-gray-200 rounded-md transition-colors" title="Math Help &amp; Examples" data-svelte-h="svelte-71cyf5">🔢</button> ${availableVoices.length > 0 ? `<button class="p-1.5 text-gray-600 hover:bg-gray-200 rounded-md transition-colors disabled:opacity-50" ${isReadingAloud ? "disabled" : ""} title="Read document aloud">${escape(isReadingAloud ? "🔊" : "📖")}</button>` : ``} ${currentDocument ? `<button class="p-1.5 text-gray-600 hover:bg-gray-200 rounded-md transition-colors" title="Version History" data-svelte-h="svelte-1mr8tj4">🕐</button>` : ``}</div></div></div></div></div>`;
});
class MathService {
  constructor() {
    this.baseURL = "/api/v1";
...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  do {
    $$settled = true;
    $$result.head = previous_head;
    $$rendered = `  ${$$result.head += `<!-- HEAD_svelte-1pchwvm_START -->${$$result.title = `<title>${escape(documentTitle)} - TPT Text Editor</title>`, ""}<!-- HEAD_svelte-1pchwvm_END -->`, ""} <div class="h-screen flex flex-col bg-white"> <div class="flex-1 overflow-hidden flex flex-col">${validate_component(TextEditor, "TextEditor").$$render(
      $$result,
      { editorMode },
      {
        editorMode: ($$value) => {
