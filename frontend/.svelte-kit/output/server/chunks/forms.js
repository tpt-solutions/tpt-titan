...
import { f as formulaBar, s as selectedCell, i as isFormulaRangeSelecting } from "./spreadsheet.js";
...
const FormulaBar = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return ` <div class="flex items-center px-4 py-2 border-b border-gray-200 bg-white"><span class="text-sm font-medium text-gray-600 mr-2 w-16">${escape($selectedCell ? getCellId($selectedCell.row, $selectedCell.col) : "")}</span> <div class="flex-1 relative"><input placeholder="Enter formula or value" class="w-full px-3 py-1 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent svelte-qenbhy"${add_attribute("value", $formulaBar, 0)}> ${$isFormulaRangeSelecting ? `<div class="absolute right-2 top-1/2 transform -translate-y-1/2 text-xs text-green-600 font-medium bg-green-100 px-2 py-1 rounded" data-svelte-h="svelte-1v6alhh">Click and drag to select range</div>` : ``}</div> </div>`;
});
export {
  ContactForm as C,
  EventForm as E,
